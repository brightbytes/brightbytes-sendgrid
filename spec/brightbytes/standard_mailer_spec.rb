...
describe StandardMailer do
  ...
  describe "when sendgrid not engaged" do
    ...
    it 'header should not be set' do
      expect(header).to_not include('X-SMTPAPI:')
    end
  end
  ...
  describe "when unsubscribe required" do
    ...
    it 'unsubscribe html version should be set' do
      expect(header).to include('"{{unsubscribe_html}}": ["{{unsubscribe_html_section}}"]')
    end

    it 'unsubscribe text version should be set' do
      expect(header).to include('"{{unsubscribe_text}}": ["{{unsubscribe_text_section}}"]')
    end

    it 'unsubscribe url should be set' do
      expect(header).to include('"{{unsubscribe_url}}": ["http://example.com/u?email=email%40email.com&category=unsubscribe"]')
    end
  end
